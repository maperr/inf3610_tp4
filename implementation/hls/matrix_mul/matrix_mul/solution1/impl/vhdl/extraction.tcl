...
proc dump_report_xml { module out } {
    ...
    set targetcp [format "%.3f" [extract_clk_period_ucf ${module}.ucf]]
    set t1_tmp [extract_timing ${module}.twr]
    if {$t1_tmp == "NA"} {
...
proc dump_report_rodin { module out device} {
    ...
    set x5 [format "%-10s %8d" BRAM: [lindex $area_results 4]]
    set x7 [format "%-10s %8d" SRL: [lindex $area_results 6]]
    set csf [glob -nocomplain *.ucf *.xdc]
    set fext [file extension $csf]
    if {$fext == ".ucf" } {
        set t2 [format "%2.3f" [extract_clk_period_ucf ${module}.ucf]]
    } else {
...
proc dump_report_rodin_xml { module out } {
    ...
    set fext [file extension $csf]
    if {$fext == ".ucf" } {
        set targetcp [format "%2.3f" [extract_clk_period_ucf ${module}.ucf]]
    } else {
        set targetcp [format "%2.3f" [extract_clk_period_sdc ${module}.xdc]]
    }
    #set t1tmp0 [extract_timing_rodin ./report/${module}_post_route.sta]
    #set t1tmp0 [extract_timing_rodin ./report/${module}_par_timing.rpt]
    set t1tmp0 [extract_timing_rodin ./report/${module}_timing_routed.rpt]
