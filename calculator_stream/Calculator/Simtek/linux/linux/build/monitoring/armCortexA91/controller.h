...
template <typename T> inline eSpaceStatus ModuleRead_1TO0_644(unsigned char ucDestinationID, unsigned long ulTimeout, T* ptData32, unsigned long ucNbElements = 1)
{
	int fd;
	unsigned long ulDataLength8=ucNbElements*sizeof(T);
...
template <typename T> inline eSpaceStatus ModuleRead_1TO6_658(unsigned char ucDestinationID, unsigned long ulTimeout, T* ptData32, unsigned long ucNbElements = 1)
{
	int fd;
	unsigned long ulDataLength8=ucNbElements*sizeof(T);
...
template <typename T> inline eSpaceStatus ModuleRead_1TO4_672(unsigned char ucDestinationID, unsigned long ulTimeout, T* ptData32, unsigned long ucNbElements = 1)
{
	int fd;
	unsigned long ulDataLength8=ucNbElements*sizeof(T);
...
template <typename T> inline eSpaceStatus ModuleRead_1TO2_686(unsigned char ucDestinationID, unsigned long ulTimeout, T* ptData32, unsigned long ucNbElements = 1)
{
	int fd;
	unsigned long ulDataLength8=ucNbElements*sizeof(T);
...
inline eSpaceStatus Memory2Stream_22TO21_631(unsigned long memorySourceAddress, unsigned char moduleDestinationID, unsigned long ulTimeout, unsigned long nbBytes)
{
	int fd; 
	...
	ioctl(fd, SPACE_DMA_VIRTUAL, 0);
	unsigned char* pcData8 = (unsigned char*)memorySourceAddress;
	ssize_t nb_transferred;
	eSpaceStatus status = SPACE_OK;
	do {
		nb_transferred = write(fd, (void*)pcData8, nbBytes);
		if(nb_transferred == -1) {
			if (errno == EINTR) {
				continue;
			} else if (errno == EAGAIN) {
		...
		nbBytes -= nb_transferred;
		pcData8 += nb_transferred;
	} while(nbBytes != 0 && nb_transferred != 0);
	close(fd);
	return status;
}

inline eSpaceStatus Stream2Memory_21TO22_698(unsigned char moduleSourceID, unsigned long memoryDestinationAddress, unsigned long ulTimeout, unsigned long nbBytes)
{
	int fd; 
	...
	ioctl(fd, SPACE_DMA_VIRTUAL, 0);
	unsigned char* pcData8 = (unsigned char*)memoryDestinationAddress;
	ssize_t nb_transferred;
	eSpaceStatus status = SPACE_OK;
	do {
		nb_transferred = read(fd, (void*)pcData8, nbBytes);
		if(nb_transferred == -1) {
			if (errno == EINTR) {
				continue;
			} else if (errno == EAGAIN) {
